...
/**
 * Context for the `*cngxTabIcon` template. Rendered once per tab in the
 * header, ahead of the label. Carries the tab handle plus the two
 * per-tab signals a consumer icon usually keys off - whether the tab is
 * the active one and its positional index. Richer than
 * {@link CngxTabErrorBadgeContext} (which only needs `tab`) because an
...
/**
 * Structural slot for the tab-icon template. One group-level slot with
 * per-tab context (stepper-slot parity), not a per-`CngxTab` input.
 * 3-stage cascade: per-instance directive >
 * `CNGX_TABS_CONFIG.templates.icon` > none (no icon rendered).
 *
 * ```html
 * <cngx-tab-group iconLayout="start">
 *   <ng-template cngxTabIcon let-tab="tab" let-active="active">
