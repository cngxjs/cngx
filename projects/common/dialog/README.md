...
Every dialog walks through `closed → opening → open → closing → closed`. The transition states are first-class — they are not internal animation bookkeeping that consumers should ignore.
...
- Live demos under `dev-app/src/app/demos/common/dialog/` cover declarative, programmatic, submit-action, bottom sheet, draggable, and nested patterns.
