...
const SIZE_FLOOR_HOSTS: ReadonlyArray<{ file: string; note: string }> = [
  ...
  // @cngx/ui - sidenav (col-resize separator; the cursor is outside the scan's
  // pointer/grab heuristic, enrolled manually)
  { file: 'projects/ui/sidenav/sidenav.css', note: 'resize separator hit zone' },
];
...
describe('touch-target size-floor coverage manifest', () => {
  ...
  it('fixes the manifest size so a bulk edit dropping several hosts is caught', () => {
    ...
    expect(SIZE_FLOOR_HOSTS.length).toBe(51);
  });
});
