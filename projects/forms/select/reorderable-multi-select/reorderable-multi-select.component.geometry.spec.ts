...
@Component({
  ...
  template: `<cngx-reorderable-multi-select [label]="'Colour'" [options]="options" [(values)]="values" />`,
})
class Host {
...
describe('CngxReorderableMultiSelect geometry', () => {
  ...
  it('lays the trigger out as a flex row with the caret pinned intrinsic', () => {
    ...
    expect(
      computedValue(query(host, '.cngx-reorderable-multi-select__caret'), 'flex-grow'),
    ).toBe('0');
  });
});
