...
export const STORY: DemoSpec = {
  ...
  description:
    'The segment is a trigger plus a shown/total readout - it holds no accumulation state. The host derives the cumulative slice from the longhand page pairs (<code>[pageIndex]</code> + <code>(pageIndexChange)</code>, <code>[pageSize]</code> + <code>(pageSizeChange)</code>; the brain also exposes <code>cumulativeRange()</code> for the same math). On the last page the button disables and its label switches to the all-loaded state, so assistive tech never hears a "Load more" action on a dead control.',
  level: 'organism',
  audience: ['dev'],
  artifact: 'standalone',
