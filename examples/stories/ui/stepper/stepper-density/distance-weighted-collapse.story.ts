...
export const STORY: DemoSpec = {
  ...
  subtitle:
    'With <code>density: \'auto\'</code> each label\'s budget shrinks with its distance from the active step. Move the active step and narrow the frame: the labels furthest from it collapse first while the active one and its neighbours stay readable.',
  description:
    'The continuous label budget subtracts a penalty proportional to each step\'s distance from the active one, so collapse fans out from the active step rather than running left to right. With the active step in the middle of a long wizard, the first and last labels truncate first as the frame narrows, the immediate neighbours hold on longer, and the active label is always kept. Step with Previous / Next and the budget re-centres on the new active step - the nearest labels grow back as the furthest give way. Collapsed labels stay in the accessibility tree (clipped, never removed), so every step button keeps its accessible name, and the strip truncates in flow instead of growing a horizontal scrollbar. Tune the falloff with <code>--cngx-step-distance-penalty</code>.',
  level: 'organism',
  audience: ['dev', 'design'],
  artifact: 'standalone',
  focus: ['behavior'],
