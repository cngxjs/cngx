...
export const STORY: DemoSpec = {
  ...
  description:
    'Pessimistic mode pins the active tab to the resolved route: while the editor form is dirty the guard refuses the switch, so the active tab never moves. The editor binds <code>[error]</code> to a flag set only when a leave is actually blocked, so unsaved text alone does not raise an error - the badge appears on a refused switch and clears on save. The unsaved editor is the problem, so the demo flags it and clears cngx\'s generic rejection marker from the tab you tried to reach (a leave-guard fault belongs on the source, not the target). Production wiring lives in the route config and the directive: <code>&lt;cngx-tab-group cngxTabsRouteSync&gt;</code> supplies the router commit-action via the <code>CNGX_TABS_COMMIT_ACTION</code> DI fallback and pins pessimistic mode, while each child route carries <code>canDeactivate: [unsavedChangesGuard]</code>. The presenter\'s commit lifecycle is the gate - <code>createTabRouterCommit()</code> resolves <code>true</code> on <code>NavigationEnd</code> and <code>false</code> on <code>NavigationCancel</code> / <code>NavigationError</code>.',
  level: 'organism',
  audience: ['dev', 'a11y'],
  artifact: 'standalone',
  ...
  moduleImports: [
    "import { CngxTab, CngxTabContent, type CngxTabGroupPresenter, type CngxTabsCommitAction } from '@cngx/common/tabs';",
  ...
  // Set only when a leave is actually refused - unsaved text alone is
  // not an error, so the badge stays off until the guard blocks a switch.
  protected readonly blocked = signal(false);
  protected readonly group = viewChild<CngxTabGroupPresenter>('tg');
   ...
   * target.
   *
   * The refusal flags the EDITOR (the unsaved tab is the problem), not
   * the tab you tried to reach. cngx pins its generic rejection marker on
   * the refused target, which is the wrong tab for a leave-guard, so we
   * clear it once it is set - leaving the editor's [error] as the only
   * signal.
   */
  protected readonly guardedSwitch: CngxTabsCommitAction = (from) => {
    const allowed = !(from === 0 && this.isDirty());
    if (!allowed) {
      this.blocked.set(true);
      queueMicrotask(() => this.group()?.clearLastFailed());
    }
    return allowed;
  };
  ...
  >
    <div cngxTab [label]="'Editor'" [error]="blocked() ? 'Unsaved changes - save before leaving' : ''">
  ...
  <p style="margin-bottom:12px">
    Type in the editor - that is just unsaved, no error yet. Now try another tab:
    the switch is refused, the active tab stays put, and the editor is flagged
    because leaving was blocked (the tab you clicked stays clean). Save to clear
    the form, then the switch succeeds.
  </p>`,
  templateChrome: `
