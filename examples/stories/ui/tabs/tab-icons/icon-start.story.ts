...
export const STORY: DemoSpec = {
  title: 'CngxTabGroup: icon-start layout',
  subtitle:
    'A single group-level <code>*cngxTabIcon</code> template renders an icon before each label, in a row. The slot context is <code>{ tab, active, index }</code> - here the glyph fills when its tab is active and switches shape by index. Default layout; set <code>iconLayout="start"</code> explicitly or omit it.',
  description:
    'Icon-layout axis: <code>start</code> (icon before label, row). The icon is one group-level template slot with per-tab context, not a per-tab input (stepper-slot parity). The icon element is <code>aria-hidden</code>; the label carries the accessible name. Orthogonal to skin and orientation.',
  level: 'organism',
  audience: ['dev', 'design'],
  artifact: 'standalone',
  ...
  template: `  <cngx-tab-group iconLayout="start" [(activeIndex)]="active" aria-label="Workspace - icon start">
