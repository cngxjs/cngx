...
export const STORY: DemoSpec = {
  ...
  description:
    'The nav points at three sections of this documentation app, and the page you are reading sits at <code>/ui/tabs/tab-nav/section-deep-link</code> - a child of the Tab nav section. No click is needed: <code>[cngxTabsRouteSync]</code> resolves the owning section from the URL when it mounts, which is what a deep link or a reload exercises, and the Tab nav link carries <code>aria-current="page"</code>. The suffix default a tablist uses would resolve nothing here, because the URL\'s trailing segment matches no link - the read returns null, the seed is a no-op, and the first link keeps <code>aria-current</code> from the initial index. That is the whole reason the nav flavour defaults to prefix. When several sections match, the longest route wins, so a <code>/ui/tabs</code> link could not shadow <code>/ui/tabs/tab-nav</code>. Set <code>match="suffix"</code> on the nav to opt an instance back into leaf matching.',
  level: 'organism',
  audience: ['dev', 'a11y'],
  artifact: 'standalone',
