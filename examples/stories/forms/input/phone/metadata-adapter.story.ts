...
export const STORY: DemoSpec = {
  ...
  description:
    'createPrefixPhoneMetadata turns a region keyed prefix map (DE mobile via /^1[567]/, AT mobile via the real NDC set 660/664/699 and friends, written without the trunk 0) into the adapter, so you declare the prefixes that matter instead of hand-writing the region branch. The adapter sees the national subscriber digits with the dial code already stripped and no trunk 0 (the international +43 form omits it), so the prefixes match. It ships no numbering data - the consumer supplies every prefix, mirroring a real library like libphonenumber-js, which stays in the consumer dependency graph, never cngx. It is wired through viewProviders, so the strategy is scoped to this component subtree. Type a DE number starting 15/16/17 (or switch to AT and type 664/660/699 without the leading 0, since the +43 dial code is already filled in) and the grouping regroups to the 3-digit mobile alternation the moment the prefix is decisive, while the entered digits are preserved. Landline grouping per area code is out of scope for a line-type strategy - that needs as-you-type formatting from a real metadata library.',
  level: 'organism',
  audience: ['dev'],
  artifact: 'standalone',
  ...
  <div class="demo-form">
    <div class="demo-field">
      <cngx-form-field [field]="phoneField">
        ...
        <span cngxHint>Type a DE number starting 15/16/17, or an AT mobile (664, 660, 699, ... - no trunk 0) - the grouping switches to mobile immediately</span>
      </cngx-form-field>
    </div>
  </div>`,
