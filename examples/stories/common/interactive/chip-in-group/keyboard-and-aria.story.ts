...
export const STORY: DemoSpec = {
  ...
  description: '<code>CngxChipInGroup</code> is the leaf directive that turns a presentational <code>&lt;cngx-chip&gt;</code> into a role=option leaf of its surrounding chip-group. The chip never holds its own selection state: <code>selected</code> is a <code>computed()</code> reading <code>parent.isSelected(value())</code>, so the group is the single source of truth. Without a parent <code>&lt;cngx-chip-group&gt;</code> or <code>&lt;cngx-multi-chip-group&gt;</code> the directive throws <code>NullInjectorError</code> at construction by design; the leaf is not standalone-interactive on purpose (use <code>[cngxChipInteraction]</code> for that). The third chip below is per-chip disabled and uses <code>[cngxDescribedBy]</code> to point at a visible reason span so screen-reader users hear why the option cannot be picked.',
  level: 'atom',
  audience: ['dev', 'a11y'],
  ...
  <div class="event-grid" style="margin-top:12px">
    <div class="event-row">
      ...
      <span class="event-value">{{ size() ?? '—' }}</span>
    </div>
  </div>`,
};
