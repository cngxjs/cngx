...
export const STORY: DemoSpec = {
  title: 'CngxErrorRegistry: Inject error aggregator programmatic',
  ...
  <section role="region" aria-labelledby="iea-heading" style="display:flex; flex-direction:column; gap:8px; max-width:28rem">
    ...
    @if (aggregator.shouldShow()) {
      <ul role="alert" style="margin:0; padding-inline-start:1.25rem">
        @for (entry of aggregator.activeErrors(); track entry.key) {
  ...
  <span class="cngx-sr-only" aria-live="polite" aria-atomic="true">{{ aggregator.announcement() }}</span>`,
  templateChrome: `
