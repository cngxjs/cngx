...
export const STORY: DemoSpec = {
  ...
  description:
    'Single-select button-toggle group with W3C APG radiogroup semantics. Tab enters the group on the active toggle (or the first one if none); arrow keys MOVE focus AND auto-select the next toggle via a transient <code>pendingArrowSelect</code> flag the leaf consumes in its <code>(focus)</code> handler. Tab-into-group never auto-selects (no preceding arrow keydown). The leaf <code>&lt;button cngxButtonToggle&gt;</code> binds <code>aria-checked</code>; mode is chosen at injection time by which parent token resolves, never via a runtime flag.',
  level: 'molecule',
