...
// Source: dev-app/src/app/demos/forms/single-select-demo/single-select-demo.story.ts#section[16] "Autofocus on mount"
...
@Component({
  selector: 'app-single-select-autofocus-on-mount',
...
export class SingleSelectAutofocusOnMount {
  ...
  protected readonly _exSectionTitle: string = 'Autofocus on mount';
  protected readonly _exSubtitle: string = '<code>[autofocus]="true"</code> focuses the trigger on first render — evaluated once, later bound changes have no effect (matches native <code>&lt;select autofocus&gt;</code>).';
...
protected toggleAutofocus(): void { this.autofocusVisible.update(v => !v); }`;
  protected readonly _exHtml: string = `<button type="button" class="chip" (click)="toggleAutofocus()" style="margin-bottom:8px">
  {{ autofocusVisible() ? 'Hide select' : 'Mount select (autofocus)' }}
</button>
@if (autofocusVisible()) {
  <cngx-select
