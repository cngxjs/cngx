...
// Source: dev-app/src/app/demos/forms/single-select-demo/single-select-demo.story.ts#section[27] "commitErrorDisplay variants — banner / inline / none"
...
@Component({
  selector: 'app-single-select-commiterrordisplay-variants-banner-inline-none',
...
export class SingleSelectCommiterrordisplayVariantsBannerInlineNone {
  ...
  protected readonly _exSectionTitle: string = 'commitErrorDisplay variants — banner / inline / none';
  protected readonly _exSubtitle: string = 'Three identical selects bound to the same fail-on-demand <code>commitAction</code> with the three <code>commitErrorDisplay</code> values side by side. Toggle <em>Fail next</em>, click a value, and watch how each display mode surfaces the rejection.';
