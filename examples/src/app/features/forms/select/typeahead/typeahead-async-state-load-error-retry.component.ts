...
// Source: dev-app/src/app/demos/forms/typeahead-demo/typeahead-demo.story.ts#section[2] "Typeahead — async [state] (load + error + retry)"
...
@Component({
  selector: 'app-typeahead-typeahead-async-state-load-error-retry',
...
export class TypeaheadTypeaheadAsyncStateLoadErrorRetry {
  ...
  protected readonly _exSectionTitle: string = 'Typeahead — async [state] (load + error + retry)';
  protected readonly _exSubtitle: string = 'Typeahead with <code>[state]</code> driving the panel view. Trigger <em>Load</em> / <em>Error</em> / <em>Reset</em> to step through the async-view machine — first-load skeleton, error banner with retry, refresh shimmer.';
