...
// strip collapses to <cngx-text-stepper>; at desktop width the strip
// renders normally.

test.describe('ui/stepper/stepper-mobile-collapse', () => {
  test('auto-collapse: 375px viewport renders cngx-text-stepper instead of the strip', async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 720 });
    await gotoDemo(page, 'ui/stepper/stepper-mobile-collapse/auto-collapse');

    await expect(page.locator('cngx-text-stepper')).toBeVisible();
    await expect(page.locator('cngx-text-stepper .cngx-text-stepper__text')).toContainText(
      'Step 1 of 3',
    );
  ...
  test('desktop viewport keeps the classic strip rendered', async ({ page }) => {
    ...
    await expect(page.locator('cngx-text-stepper')).toHaveCount(0);
