...
@Component({
  ...
  template: `
    <app-doc-shell title="Chart Primitives"
      ...
      overview="<p>Every preset molecule is a thin wrapper over <code>&lt;cngx-chart&gt;</code> + axes + layer atoms. When the presets do not fit a use case (multi-layer dashboards, custom A11y wiring, mixed visualisations), compose the primitives directly. The full two-level CSS cascade and reactive ARIA contract is identical.</p><p>Layer atoms and <code>[cngxAxis]</code> are attribute directives applied to <code>&lt;svg:g&gt;</code> hosts so the SVG namespace boundary stays clean. Element selectors inside <code>&lt;svg&gt;</code> would create XHTML-namespaced wrappers and break layout for their SVG-namespaced children.</p>"
  ...
  >
    ...
    <ng-template cngxChartError let-err="error" let-small="small">
      ...
      <app-example-card title="Responsive (fills parent width)"
        [subtitle]="_s5"
