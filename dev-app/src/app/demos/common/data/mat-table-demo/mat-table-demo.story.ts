...
// &lt;mat-table [dataSource]="ds"&gt; — same as any CDK DataSource</code></pre>`,
    ...
    {
      title: 'SmartDataSource + mat-table — auto-wired Sort & Filter',
      subtitle:
        '<code>CngxSort</code> and <code>CngxFilter</code> are declared as <code>hostDirectives</code> on the component. ' +
        '<code>injectSmartDataSource()</code> discovers them automatically via <code>inject()</code> — no explicit wiring needed. ' +
        "<code>CngxSmartDataSource</code> extends CDK's <code>DataSource&lt;T&gt;</code>, so <code>mat-table</code> accepts it directly as <code>[dataSource]</code>. " +
        'Multi-filter is fully supported — all named predicates are AND-combined automatically. ' +
        'Multi-sort is also supported — add <code>[multiSort]="true"</code> to the <code>[cngxSort]</code> element and Shift+click column headers.',
      imports: ['CngxSortHeader', 'MatTableModule'],
      template: `
  <div class="filter-row">
  ...
  <div class="status-row">
    ...
    {
      title: 'Why CngxSearch does not auto-wire',
      subtitle:
        '<code>inject()</code> traverses the injector tree <em>upward</em> — from the DataSource context to the component and its ancestors. ' +
        '<code>CngxSearch</code> lives on an <code>&lt;input&gt;</code> element inside the template (a child element), which is <em>below</em> the component injector. ' +
        'For search integration use <code>CngxDataSource</code> with a manual <code>computed()</code> — shown in the next demo.',
      template: `
