...
export const STORY: DemoSpec = {
  ...
  sections: [
    {
      ...
      subtitle:
        '<code>[cngxReducedMotion]</code> reflects <code>prefers-reduced-motion: reduce</code> ' +
        'as a signal. Use it in TypeScript to conditionally skip animations, transitions, ' +
        'or auto-playing media — not just CSS. The <code>cngx-reduced-motion</code> class ' +
        'on the host element enables CSS-only overrides too.',
  ...
  <style>
    ...
    },
    {
      ...
      subtitle:
        'Notifications that slide in with animation, or appear instantly when ' +
        '<code>prefersReducedMotion()</code> is <code>true</code>. ' +
        'This demonstrates using the signal in TypeScript logic, not just CSS.',
      imports: ['CngxReducedMotion'],
      template: `
