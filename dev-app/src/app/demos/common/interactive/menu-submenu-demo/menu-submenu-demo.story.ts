...
export const STORY: DemoSpec = {
  title: 'Menu Submenu',
  navLabel: 'Menu Submenu',
  navCategory: 'interactive',
  description:
    'Nested menu with cngxMenuItemSubmenu. ArrowRight on a submenu parent opens the inner menu and transfers AD focus to its first item; ArrowLeft / Escape close. Two-level keyboard navigation through one trigger button.',
  apiComponents: [
    'CngxMenu',
    'CngxMenuItem',
    'CngxMenuItemSubmenu',
  ...
  overview:
    ...
    '<ul>' +
    '<li><code>[cngxMenuItemSubmenu]="popoverRef"</code> — the inner <code>CngxPopover</code> wrapping the submenu.</li>' +
    '<li><code>[submenuMenu]="menuRef"</code> — the inner <code>CngxMenu</code> the trigger transfers focus to when the submenu opens.</li>' +
    '</ul>' +
    '<p>The inner popover MUST set <code>[exclusive]="false"</code> — opening it would otherwise light-dismiss the parent. See <code>menu-accepted-debt.md §1</code> for the rationale.</p>' +
    '<p>Keyboard model: <kbd>ArrowDown</kbd>/<kbd>Up</kbd> to navigate items, <kbd>ArrowRight</kbd> on a submenu-parent opens the submenu and highlights its first item, <kbd>ArrowLeft</kbd> closes the submenu and returns focus to the parent, <kbd>Escape</kbd> closes the top of the focus stack.</p>',
  moduleImports: [
  ...
  sections: [
    {
      ...
      subtitle:
        'Open the menu (ArrowDown / Enter / Space). Highlight "Open Recent" and press ArrowRight to open the submenu — focus transfers to its first file. ArrowLeft / Escape close. Activating a leaf closes everything.',
      imports: [
        'CngxMenu',
        'CngxMenuItem',
        'CngxMenuItemSubmenu',
