...
@Component({
  ...
  template: `
    <app-doc-shell title="Material Integration"
      ...
      overview="<p><code>[cngxBindField]</code> is the universal bridge from any control — Material, native HTML, or a custom component — to <code>&lt;cngx-form-field&gt;</code>. Place it on the same element as the Material control; cngx derives <code>id</code>, <code>aria-*</code>, <code>empty</code>, <code>focused</code>, <code>disabled</code>, and <code>errorState</code> purely from the bound <code>Field</code> via the presenter.</p><p>Value-flow goes through the control's own bindings — <code>[control]</code> (Signal Forms) or <code>[formControl]</code> (Reactive Forms). cngx never touches the value layer.</p><p>An optional Material theme mixin (<code>@cngx/forms/field/material-theme</code>) maps M3/M2 design tokens to cngx-form-field CSS custom properties.</p>"
