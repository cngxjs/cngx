...
export const STORY: DemoSpec = {
  title: 'Form Primitives — write the atom once, pick your forms style',
  ...
  overview:
    ...
    '<p><strong>What you do NOT write.</strong> No <code>ControlValueAccessor</code> on the atom. No bridge directive on the consumer side. No per-atom CVA boilerplate. No conditional template logic for "Signal Forms vs RF" — both paradigms target the same atom shape.</p>' +
    '<p><strong>How it works (briefly).</strong> Each atom exposes its value through a small contract token (<code>CNGX_CONTROL_VALUE</code>). The Reactive Forms bridge (<code>CngxFormBridge</code>) and the Signal Forms field-presenter (<code>&lt;cngx-form-field&gt;</code>) both read that token and adapt to their paradigm. Two consumer paths, one atom layer. See the README at <code>projects/forms/controls/README.md</code> for the full architecture.</p>' +
    '<p><strong>Note on <code>CngxChipInput</code>.</strong> The tokenizer chip-input does not fit this bridge — it emits <code>tokenCreated</code> events instead of owning a <code>value</code> model (the consumer holds the chip list). A dedicated bridge ships in a follow-up. Section 3 below stays as a placeholder.</p>',
  moduleImports: [
  ...
  <div class="event-grid" style="margin-top:16px">
...

    {
      ...
      subtitle:
        '<code>CngxChipInput</code> (the tokenizer that emits <code>tokenCreated</code> events as the user types) does not fit the same bridge as the nine controls above — it has no single <code>value</code> to bind because the consumer holds the chip list externally. A dedicated bridge ships in a follow-up so consumers can drop <code>&lt;input cngxChipInput formControlName="tags"&gt;</code> next to the existing <code>tokenCreated</code> output. Until then, wire the input manually as you would today.',
      imports: [],
      template: `
