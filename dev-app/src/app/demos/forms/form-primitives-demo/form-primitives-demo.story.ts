...
export const STORY: DemoSpec = {
  ...
  overview:
    '<p>Phase 7 closure of the form-primitives system. Atoms in <code>@cngx/common/interactive</code> are Forms-agnostic at Level 2 — they expose <code>CNGX_CONTROL_VALUE</code> for value flow and <code>CNGX_FORM_FIELD_CONTROL</code> for ARIA discovery. Two independent paths consume those contracts:</p>' +
    '<ul>' +
    '<li><strong>Signal Forms</strong> — drop the atom inside <code>&lt;cngx-form-field [field]="f.x"&gt;</code>. The presenter discovers the atom via <code>CNGX_FORM_FIELD_CONTROL</code>; the new <code>CNGX_FORM_FIELD_HOST</code> token (Phase 7 commit 2a) lets the atom react to <code>showError()</code> and call <code>markAsTouched()</code> on focusout — entirely Sheriff-clean across the layer boundary.</li>' +
    '<li><strong>Reactive Forms</strong> — bind <code>[formControl]</code> or <code>[formControlName]</code> on the atom. <code>CngxFormBridge</code> in <code>@cngx/forms/controls</code> auto-attaches via element selector, providing <code>NG_VALUE_ACCESSOR</code> and bidirectionally syncing the <code>FormControl</code> with the atom\'s <code>value</code> model.</li>' +
    '</ul>' +
    '<p><code>CngxChipInput</code> is deferred to Phase 7.x (its tokenizer shape — emits <code>tokenCreated</code> events, no <code>value</code> model — needs a separate <code>CngxChipInputFieldBridge</code> modelled on <code>CngxListboxFieldBridge</code>).</p>',
  moduleImports: [
  ...
  sections: [
    {
      title: 'Signal Forms — boolean + scalar + array shapes',
      subtitle:
        'Atom drops inside <code>&lt;cngx-form-field [field]="f.x"&gt;</code>. The presenter discovers the atom via <code>CNGX_FORM_FIELD_CONTROL</code>. Required-validator messages auto-render through <code>&lt;cngx-field-errors&gt;</code> once the field is touched.',
      imports: [
        'CngxFormField',
        'CngxLabel',
        'CngxFieldErrors',
  ...
  <div style="display:grid;gap:16px;max-width:480px">
    ...
    {
      title: 'Reactive Forms — CngxFormBridge auto-attaches',
      subtitle:
        'Bind <code>[formControl]</code> on any of the nine atoms; <code>CngxFormBridge</code> from <code>@cngx/forms/controls</code> matches the element selector and provides <code>NG_VALUE_ACCESSOR</code>. No bridge directive on the consumer side.',
      imports: [
        'ReactiveFormsModule',
        'CngxFormBridge',
        'JsonPipe',
