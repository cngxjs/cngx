...
// Source: demos/forms/combobox-demo/combobox-demo.story.ts
...
@Component({
  ...
  template: `
    <app-doc-shell title="Combobox"
      ...
      [apiComponents]="['CngxCombobox', 'CngxComboboxChip', 'CngxComboboxTriggerLabel']">
...
export class ComboboxDemoComponent {
  protected readonly _s0 = '<code>&lt;cngx-combobox&gt;</code> — inline <code>&lt;input role="combobox"&gt;</code> next to the chip strip. Typing filters the panel live; Backspace on an empty input removes the trailing chip; panel stays open on each pick (<code>closeOnSelect</code> default <code>false</code>).';
  protected readonly _s1 = 'Async-option wiring for server-driven autocomplete. <code>[skipInitial]="true"</code> suppresses the hydrate-time empty-string emission so your request handler never fires a "load everything" on mount. <code>(searchTermChange)</code> bridges the debounced term to your backend; <code>[state]</code> feeds the returned options back into the panel with the full async-view protocol (loading/empty/error/refreshing).';
  protected readonly _s2 = 'Every toggle (option click, chip ×, Backspace-on-empty, clear-all) routes through an async write with optimistic/pessimistic supersede semantics — same wiring as the multi-select producer.';
  protected readonly _s3 = 'Reuse the shared <code>*cngxSelectClearButton</code> slot to swap the default ✕ for any consumer-authored trigger. Same slot works on <code>CngxSelect</code> and <code>CngxMultiSelect</code>.';
  protected readonly _s4 = 'Replace the chip strip with a plain-text summary while keeping the filter input visible. Context exposes the resolved options, raw values, and count — ideal for compact variants ("3 topics selected" + input on the same row).';
  protected readonly _s5 = 'Per-chip override for the combobox\'s tag strip — same context shape as <code>*cngxMultiSelectChip</code> (<code>{ option, remove, index }</code>), so a consumer-authored chip template can be projected into either variant unchanged.';
  protected readonly _srcHtml0 = `<cngx-combobox
    [label]="'Topics'"
    [options]="tagOptions"
    [(values)]="comboValues"
    placeholder="Search topics…"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml1 = `<cngx-combobox
    [label]="'Topics'"
    [state]="comboAsyncState"
    [(values)]="comboAsyncValues"
    [skipInitial]="true"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml2 = `<div class="event-row" style="gap:8px;align-items:center;margin-bottom:8px">
    <button type="button" class="chip"
            [style.background]="comboCommitMode() === 'optimistic' ? 'color-mix(in oklch, var(--cngx-color-success) 18%, transparent)' : ''"
            (click)="comboCommitMode.set('optimistic')">optimistic</button>
    <button type="button" class="chip"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml3 = `<cngx-combobox
    [label]="'Topics'"
    [options]="tagOptions"
    [clearable]="true"
    [(values)]="comboClearableValues"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml4 = `<cngx-combobox
    [label]="'Topics'"
    [options]="tagOptions"
    [(values)]="comboTextValues"
    placeholder="Search topics…"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml5 = `<cngx-combobox [label]="'Topics'" [options]="tagOptions" [(values)]="comboValues" placeholder="Choose tag…">
    <ng-template cngxComboboxChip let-opt let-remove="remove" let-i="index">
      <span style="display:inline-flex;align-items:center;gap:0.25rem;padding:0.15rem 0.5rem;border-radius:999px;background:color-mix(in oklch, var(--cngx-color-info) 15%, transparent);color:var(--cngx-color-info);font-size:0.8rem">
        <span aria-hidden="true">#{{ i + 1 }}</span>
        <strong>{{ opt.label }}</strong>
