...
// Source: demos/forms/tree-select-demo/tree-select-demo.story.ts
...
@Component({
  ...
  template: `
    <app-doc-shell title="Tree Select"
      description="CngxTreeSelect — hierarchical multi-value picker with W3C APG treeview ARIA, cascade-children opt-in, and indeterminate-parent propagation."
      overview="<p><code>&lt;cngx-tree-select&gt;</code> composes <code>CngxTreeController</code> + <code>SelectionController</code> (with <code>childrenFn</code>) + <code>CngxPopover</code> + <code>CngxTreeSelectPanel</code> (which itself hosts <code>CngxActiveDescendant</code> + <code>CngxHierarchicalNav</code>) into a multi-value tree picker. Provides <code>CNGX_FORM_FIELD_CONTROL</code> directly.</p><p><strong>Selection</strong>: default mode toggles single values. With <code>[cascadeChildren]=&quot;true&quot;</code>, a parent toggle selects/deselects every descendant atomically and emits a single <code>selectionChange</code> with <code>action: 'cascade-toggle'</code>. Indeterminate-parent state propagates automatically through the selection controller's <code>childrenFn</code> — no manual sync.</p><p><strong>Keyboard</strong>: ArrowUp/Down + Home/End + typeahead via <code>CngxActiveDescendant</code>; ArrowRight expands / moves to first child, ArrowLeft collapses / moves to parent via <code>CngxHierarchicalNav</code>; Enter / Space activate the highlighted row.</p><p><strong>ARIA</strong>: reactive <code>role=&quot;tree&quot;</code> + per-row <code>role=&quot;treeitem&quot;</code> with <code>aria-level</code> / <code>-posinset</code> / <code>-setsize</code> / <code>-expanded</code> / <code>-selected</code> / <code>-disabled</code> all wired to the reactive graph.</p>"
      [apiComponents]="['CngxTreeSelect', 'CngxTreeSelectNode']">
      <app-example-card title="Basic — single-level toggle"
        [subtitle]="_s0"
  ...
  <div class="event-grid" style="margin-top:12px">
    ...
    <div class="event-row">
      ...
      <app-example-card title="Cascade children — parent toggle selects the whole subtree"
        [subtitle]="_s1"
  ...
  <div class="event-grid" style="margin-top:12px">
    <div class="event-row">
      ...
      <app-example-card title="Indeterminate propagation (pre-seeded partial selection)"
        [subtitle]="_s2"
  ...
  <div class="event-grid" style="margin-top:12px">
    <div class="event-row">
      ...
      <app-example-card title="Custom *cngxTreeSelectNode template"
        [subtitle]="_s3"
  ...
  >
    ...
    >
      ...
      <app-example-card title="Commit action (optimistic / pessimistic + rollback)"
        [subtitle]="_s4"
  ...
  <div class="event-grid" style="margin-top:12px">
    ...
    <div class="event-row">
      ...
      <app-example-card title="10.000 nodes — perf smoke"
        [subtitle]="_s5"
...
export class TreeSelectDemoComponent {
  protected readonly _s0 = 'Cascade off by default: each click toggles the individual row. Parents still receive <code>aria-indeterminate</code> via the selection controller\'s <code>childrenFn</code> — flip a leaf and watch the branch indicator change.';
  protected readonly _s1 = '<code>[cascadeChildren]="true"</code> flips every descendant atomically on a single <code>selectionChange</code> event with <code>action: \'cascade-toggle\'</code>. The chip × button still removes a single value (no hidden mass-deselect from the chip strip).';
  protected readonly _s2 = 'The selection controller\'s <code>childrenFn</code> is wired to <code>treeController.childrenOfValue</code> — any partial descendant selection surfaces as <code>aria-indeterminate</code> on the parent row with zero manual wiring. Open the panel to see <em>Lena Kovač</em> and <em>Marcus Vogel</em> show the half-state indicator.';
  ...
  protected readonly _srcTs0 = `import { CngxTreeSelect, CngxTreeSelectNode, type CngxTreeNode } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
import { ORG_TREE, type Employee } from '../../../fixtures/employees-tree.fixture';
  ...
  protected readonly _srcHtml1 = `<label style="display:inline-flex;align-items:center;gap:0.5rem;margin-bottom:0.5rem">
    <input type="checkbox" [checked]="cascadeOn()"
           (change)="cascadeOn.set($any($event.target).checked)" />
    cascadeChildren
  </label>
  ...
  protected readonly _srcTs1 = `import { CngxTreeSelect, CngxTreeSelectNode, type CngxTreeNode } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
import { ORG_TREE, type Employee } from '../../../fixtures/employees-tree.fixture';
  ...
  protected readonly perfKeyFn = (v: { id: string }): unknown => v.id;
  protected readonly perfValues = signal<{ id: string; label: string }[]>([]);`;
  protected readonly _srcHtml2 = `<cngx-tree-select
    [nodes]="orgTree"
    [nodeIdFn]="orgIdFn"
    [labelFn]="orgLabelFn"
    [keyFn]="orgKeyFn"
  ...
  protected readonly _srcTs2 = `import { CngxTreeSelect, CngxTreeSelectNode, type CngxTreeNode } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
import { ORG_TREE, type Employee } from '../../../fixtures/employees-tree.fixture';
  ...
  protected readonly perfKeyFn = (v: { id: string }): unknown => v.id;
  protected readonly perfValues = signal<{ id: string; label: string }[]>([]);`;
  protected readonly _srcHtml3 = `<cngx-tree-select
    [nodes]="orgTree"
    [nodeIdFn]="orgIdFn"
    [labelFn]="orgLabelFn"
    [keyFn]="orgKeyFn"
  ...
  protected readonly _srcTs3 = `import { CngxTreeSelect, CngxTreeSelectNode, type CngxTreeNode } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
import { ORG_TREE, type Employee } from '../../../fixtures/employees-tree.fixture';
  ...
  protected readonly perfKeyFn = (v: { id: string }): unknown => v.id;
  protected readonly perfValues = signal<{ id: string; label: string }[]>([]);`;
  protected readonly _srcHtml4 = `<div class="event-row" style="gap:8px;align-items:center;margin-bottom:8px">
    <button type="button" class="chip"
            [style.background]="commitMode() === 'optimistic' ? '#c8e6c9' : ''"
            (click)="commitMode.set('optimistic')">optimistic</button>
    <button type="button" class="chip"
  ...
  protected readonly _srcTs4 = `import { CngxTreeSelect, CngxTreeSelectNode, type CngxTreeNode } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
import { ORG_TREE, type Employee } from '../../../fixtures/employees-tree.fixture';
  ...
  protected readonly perfKeyFn = (v: { id: string }): unknown => v.id;
  protected readonly perfValues = signal<{ id: string; label: string }[]>([]);`;
  protected readonly _srcHtml5 = `<cngx-tree-select
    [nodes]="perfTree"
    [nodeIdFn]="perfIdFn"
    [labelFn]="perfLabelFn"
    [keyFn]="perfKeyFn"
  ...
  protected readonly _srcTs5 = `import { CngxTreeSelect, CngxTreeSelectNode, type CngxTreeNode } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
import { ORG_TREE, type Employee } from '../../../fixtures/employees-tree.fixture';
