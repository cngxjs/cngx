...
// Source: demos/forms/single-select-demo/single-select-demo.story.ts
...
@Component({
  ...
  template: `
    <app-doc-shell title="Single Select"
      ...
      [apiComponents]="['CngxSelect', 'CngxSelectOption', 'CngxSelectOptgroup', 'CngxSelectDivider', 'CngxSelectCheck', 'CngxSelectCaret', 'CngxSelectOptgroupTemplate', 'CngxSelectPlaceholder', 'CngxSelectEmpty', 'CngxSelectLoading', 'CngxSelectLoadingGlyph', 'CngxSelectRefreshing', 'CngxSelectCommitError', 'CngxSelectOptionPending', 'CngxSelectOptionError', 'CngxSelectRetryButton', 'CngxSelectTriggerLabel', 'CngxSelectOptionLabel', 'CngxSelectClearButton', 'provideSelectConfig']">
...
export class SingleSelectDemoComponent {
  ...
  protected readonly _s1 = 'Composing <code>&lt;cngx-option&gt;</code> / <code>&lt;cngx-optgroup&gt;</code> / <code>&lt;cngx-select-divider&gt;</code> directly inside <code>&lt;cngx-select&gt;</code> <strong>does not work</strong> — content-projection scoping puts the projected children in <code>cngx-select</code>\'s injector tree, not the inner listbox\'s, so <code>CngxActiveDescendant</code> registration fails and the panel opens empty. They <strong>do work</strong> when you compose the listbox yourself using the Level-2 atoms (<code>CngxPopover</code> + <code>CngxListboxTrigger</code> + <code>CngxListbox</code>), because the options sit inside the listbox\'s own content-children scope.';
  protected readonly _s2 = 'Grouped options: pass an array mixing <code>CngxSelectOption</code> and <code>CngxSelectOptionGroup</code>.';
  protected readonly _s3 = '<code>[clearable]="true"</code> adds a ✕ button when a value is selected.';
  protected readonly _s4 = 'Project a <code>*cngxSelectOptionLabel</code> template to render icons/badges per option.';
  protected readonly _s5 = 'Override panel content via <code>*cngxSelectLoading</code> / <code>*cngxSelectEmpty</code>.';
  protected readonly _s6 = '<code>[state]</code> drives the panel via <code>CngxAsyncState</code>: loading → skeleton, success → options, empty → empty template, refreshing → top-bar + options, error → retry panel. Replaces <code>[options]</code> while the state has data.';
  ...
  protected readonly _s8 = '<code>[loadingVariant]</code> picks one of four built-in first-load visuals: <code>spinner</code> (default), <code>skeleton</code> (with configurable <code>[skeletonRowCount]</code>), <code>bar</code>, or <code>text</code>. Globally configurable via <code>provideSelectConfig(withLoadingVariant(\'skeleton\'), withSkeletonRowCount(5))</code>.';
  protected readonly _s9 = '<code>[refreshingVariant]</code> controls the subsequent-load indicator when options stay visible: <code>bar</code> (default), <code>spinner</code>, <code>dots</code>, or <code>none</code>. Triggered by <code>state.status() === \'refreshing\'</code>.';
  protected readonly _s10 = 'Project a <code>*cngxSelectCaret</code> template — the default ▾ glyph gets replaced with any markup you want, with <code>let-open="open"</code> available for rotation state.';
  protected readonly _s11 = 'Project <code>*cngxSelectCheck</code> to replace the ✓ glyph shown on the selected row. Context: <code>let-option</code>, <code>let-selected="selected"</code>.';
  protected readonly _s12 = '<code>[selectionIndicatorVariant]="\'radio\'"</code> swaps the panel\'s built-in indicator from the checkmark to <code>cngx-radio-indicator</code> (dot-in-circle). Useful for single-select panels that want a radio-style visual without losing dropdown ergonomics. Set globally via <code>provideSelectConfig(withSelectionIndicatorVariant(\'radio\'))</code>.';
  ...
  protected readonly _s19 = 'Replace the plain placeholder string with custom markup — render an icon + the placeholder text, a stylised hint, or a help link inside the trigger.';
  protected readonly _s20 = 'Replace the panel-shell\'s default loading indicator with a consumer-authored body — useful for branded spinners, progress text, or a cancel-and-restart affordance.';
  protected readonly _s21 = 'Re-skin grouped-option labels — render badges, icons, or counts in the optgroup header without touching the option rows themselves. Class name <code>CngxSelectOptgroupTemplate</code> distinguishes this directive from the <code>&lt;cngx-optgroup&gt;</code> element component used in declarative composition.';
  protected readonly _s22 = 'Replace the default 2px progress bar overlaid on stale options. Override receives <code>previousCount</code> so consumer templates can render context-aware status like <em>"Refreshing 4 items"</em>.';
  protected readonly _s23 = 'Replace the panel-shell\'s default commit-error banner with custom markup. Override receives <code>error</code>, the <code>option</code> the user was trying to pick, and a <code>retry()</code> callback that replays the commit.';
  protected readonly _s24 = 'Per-option-row indicators driven by <code>[commitAction]</code>. Pending shows while the commit is in flight; the error glyph appears on the row that failed (with <code>commitErrorDisplay="inline"</code>).';
  protected readonly _s25 = 'Swap the visual frame of every Retry / Try again button rendered by the shared panel-shell — load-error, inline refresh-error, and commit-error banner all read from this single override. Context: <code>{ retry, error, disabled, label }</code>.';
  protected readonly _s26 = 'Replace the inner CSS-driven glyph of the spinner / bar / dots loading variants while keeping the shell\'s ARIA wiring (<code>role="status"</code>, <code>aria-live</code>, <code>aria-label</code>). Skeleton variant ignores this slot — its rows are layout, not glyph.';
  protected readonly _s27 = 'Three identical selects bound to the same fail-on-demand <code>commitAction</code> with the three <code>commitErrorDisplay</code> values side by side. Toggle <em>Fail next</em>, click a value, and watch how each display mode surfaces the rejection.';
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml1 = `<button type="button"
          class="chip"
          [cngxPopoverTrigger]="myPop"
          [haspopup]="'listbox'"
          [cngxListboxTrigger]="myLb"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml19 = `<cngx-select [label]="'Color'" [options]="colors" [(value)]="standaloneValue" placeholder="Pick a color…">
    <ng-template cngxSelectPlaceholder let-text>
      <span style="display:inline-flex;align-items:center;gap:0.4rem;color:var(--cngx-color-text-muted)">
        <span aria-hidden="true">🎨</span>
        <em>{{ text }}</em>
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml20 = `<cngx-select [label]="'Language'" [options]="loadingOptions" [(value)]="loadingValue" [loading]="loading()" placeholder="Choose language…">
    <ng-template cngxSelectLoading let-retry="retry">
      <div role="status" aria-live="polite" style="display:flex;flex-direction:column;align-items:center;gap:0.5rem;padding:1rem">
        <span aria-hidden="true" style="font-size:1.5rem">⏳</span>
        <span>Loading available languages…</span>
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml21 = `<cngx-select [label]="'Priority'" [options]="priorities" [(value)]="groupedValue" placeholder="Choose priority…">
    <ng-template cngxSelectOptgroup let-group>
      <span style="display:inline-flex;align-items:center;gap:0.5rem">
        <span aria-hidden="true" style="font-size:0.75rem;padding:2px 6px;border-radius:999px;background:color-mix(in oklch, var(--cngx-color-text) 8%, transparent);color:var(--cngx-color-text-muted)">{{ group.children?.length ?? 0 }}</span>
        <strong>{{ group.label }}</strong>
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml22 = `<cngx-select [label]="'Language'" [options]="asyncOptions" [(value)]="asyncValue" [state]="asyncState" placeholder="Choose language…">
    <ng-template cngxSelectRefreshing let-previousCount="previousCount">
      <div role="status" aria-live="polite" style="padding:0.4rem 0.75rem;font-size:0.8rem;color:var(--cngx-color-info);background:linear-gradient(90deg,color-mix(in oklch, var(--cngx-color-info) 10%, transparent),color-mix(in oklch, var(--cngx-color-info) 25%, transparent),color-mix(in oklch, var(--cngx-color-info) 10%, transparent));background-size:200% 100%;animation:cngx-select-refresh-shimmer 1.6s linear infinite">
        🔄 Refreshing {{ previousCount }} options…
      </div>
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml23 = `<cngx-select
    [label]="'Color'"
    [options]="colors"
    [(value)]="commitValue"
    [commitAction]="commitAction"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml24 = `<cngx-select
    [label]="'Color'"
    [options]="colors"
    [(value)]="commitValue"
    [commitAction]="commitAction"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml25 = `<cngx-select [label]="'Language'" [options]="asyncOptions" [(value)]="asyncValue" [state]="asyncState" placeholder="Choose language…">
    <ng-template cngxSelectRetryButton let-retry let-label="label" let-disabled="disabled">
      <button type="button" class="chip" [disabled]="disabled" (click)="retry()" style="background:color-mix(in oklch, var(--cngx-color-warning) 10%, transparent);border-color:color-mix(in oklch, var(--cngx-color-warning) 35%, transparent);color:var(--cngx-color-warning)">
        ↻ {{ label }}
      </button>
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml26 = `<cngx-select
    [label]="'Language'"
    [options]="asyncOptions"
    [(value)]="asyncValue"
    [state]="asyncState"
...
import { createManualState, type ManualAsyncState } from '@cngx/common/data';
  ...
  protected readonly _srcHtml27 = `<div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:1rem">
    <div>
      <h4 style="margin:0 0 0.5rem 0;font-size:0.85rem">banner</h4>
      <cngx-select
        [label]="'Color'"
