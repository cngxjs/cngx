...
// Source: demos/forms/action-select-demo/action-select-demo.story.ts
...
@Component({
  selector: 'app-action-select-demo',
  standalone: true,
  ...
  template: `
    <app-doc-shell title="Action Select"
      ...
      overview="<p><code>&lt;cngx-action-select&gt;</code> mirrors <code>CngxTypeahead</code>'s inline-input autocomplete surface (<code>displayWith</code>, <code>clearOnBlur</code>, async commits, <code>searchMatchFn</code>) and adds an inline quick-create workflow via the <code>*cngxSelectAction</code> slot. Compose a template inside the panel, call its <code>commit()</code> callback, and the bound <code>[quickCreateAction]</code> materialises a new <code>T</code>. On success the new item lands in the persistent local buffer, becomes the current value, announces <code>'created'</code>, and (optionally) closes the panel.</p><p><strong>Dismiss guard:</strong> consumer templates that flip <code>setDirty(true)</code> intercept Escape and click-outside — the panel stays open until <code>cancel()</code> fires or the commit resolves. <strong>Focus trap:</strong> the shared panel shell's <code>CngxFocusTrap</code> activates when dirty (default) — configurable via <code>provideActionSelectConfig(withFocusTrapBehavior('always'|'dirty'|'never'))</code>.</p><p><strong>Commit semantics:</strong> pessimistic — the panel stays open with an <code>isPending</code> flag in the slot context so a custom button can show a spinner. Supersede applies via the shared commit controller (consecutive creates cancel the in-flight one cleanly).</p>"
      [apiComponents]="['CngxActionSelect', 'CngxSelectAction', 'provideActionSelectConfig', 'createCreateCommitHandler']">
      <app-example-card title="Basic — sync quick-create"
        [subtitle]="_s0"
  ...
  <div class="event-grid" style="margin-top:12px">
    ...
    <div class="event-row">
      ...
      <app-example-card title="Pre-seeded + (created) output log"
        [subtitle]="_s1"
  ...
  <div class="event-grid" style="margin-top:12px">
    ...
    @for (line of asyncLog(); track line) {
      ...
      <app-example-card title="Dirty guard — Escape cancel + click-outside blocked"
        [subtitle]="_s3"
  ...
  <div class="event-grid" style="margin-top:12px">
    <div class="event-row">
      ...
      <app-example-card title="Custom action template — split actions"
        [subtitle]="_s4"
...
export class ActionSelectDemoComponent {
  protected readonly _s0 = 'Type a label and press the <strong>Create</strong> button inside the panel to materialise a new entry. The sync create returns a fresh value; the new option becomes the current selection, lands in the persistent local buffer, and the panel closes.';
  protected readonly _s1 = 'Component starts with <em>Development</em> already selected. The <code>(created)</code> output fires after every successful quick-create — dedicated channel for consumers that only care about creation events without branching on <code>action === \'create\'</code>.';
  protected readonly _s2 = 'Quick-create routes through an Observable with 600ms delay. Toggle the <strong>Server fails</strong> checkbox to make the next create reject — the error surfaces via <code>(commitError)</code>, the commit-error banner renders above the options, and the value stays untouched (pessimistic flow).';
  protected readonly _s3 = 'The action slot includes a free-text description field. Typing into it calls <code>setDirty(true)</code> through the slot context; while dirty, Escape intercepts (fires <code>cancel()</code>) and click-outside is silently blocked. Press the explicit <strong>Cancel</strong> button or successful create to release the guard.';
  protected readonly _s4 = 'The action slot is just an <code>ng-template</code> with a rich context — project anything: a split button with both <em>Create &amp; close</em> and <em>Create &amp; keep open</em>, keyboard shortcuts, icons, mini-wizards. The component\'s <code>closeOnCreate</code> input is the default; consumer templates can call <code>close()</code> from the context to force-close after an <code>onCreated</code> emit.';
  ...
  protected readonly _srcTs0 = `import { CngxActionSelect, type CngxSelectCreateAction, type CngxSelectOptionDef } from '@cngx/forms/select';
import { CngxSelectAction } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
  ...
  protected readonly _srcHtml1 = `<cngx-action-select
    [label]="'Tag (pre-seeded)'"
    [options]="tags"
    [compareWith]="basicCompare"
    [displayWith]="basicDisplay"
  ...
  protected readonly _srcTs1 = `import { CngxActionSelect, type CngxSelectCreateAction, type CngxSelectOptionDef } from '@cngx/forms/select';
import { CngxSelectAction } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
  ...
  protected readonly _srcTs2 = `import { CngxActionSelect, type CngxSelectCreateAction, type CngxSelectOptionDef } from '@cngx/forms/select';
import { CngxSelectAction } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
  ...
  protected readonly _srcTs3 = `import { CngxActionSelect, type CngxSelectCreateAction, type CngxSelectOptionDef } from '@cngx/forms/select';
import { CngxSelectAction } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
  ...
  protected readonly customCreate: CngxSelectCreateAction<{ id: string; name: string }> =
    (_term, draft) => ({ id: 'custom-' + ++this.customCounter, name: draft.label });`;
  protected readonly _srcHtml4 = `<cngx-action-select
    [label]="'Custom'"
    [options]="tags"
    [compareWith]="basicCompare"
    [displayWith]="basicDisplay"
  ...
  protected readonly _srcTs4 = `import { CngxActionSelect, type CngxSelectCreateAction, type CngxSelectOptionDef } from '@cngx/forms/select';
import { CngxSelectAction } from '@cngx/forms/select';
import { delay, of, throwError } from 'rxjs';
