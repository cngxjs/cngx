...
// Source: demos/ui/tabs/tab-slot-overrides-demo/tab-slot-overrides-demo.story.ts
...
@Component({
  selector: 'app-tab-slot-overrides-demo',
  standalone: true,
  ...
  template: `
    <app-doc-shell title="Tabs — slot overrides"
      ...
      [apiComponents]="['CngxTabGroup', 'CngxTab', 'CngxTabContent', 'CngxTabErrorBadge', 'CngxTabRejectionIcon', 'CngxTabBusySpinner']">
      <app-example-card title="Custom error badge via <code>*cngxTabErrorBadge</code>"
        [subtitle]="_s0"
  ...
  <div class="event-grid" style="margin-top:var(--demo-grid-gap, 12px);gap:8px">
    ...
    <div class="event-row"><span class="event-label">Active tab</span><span class="event-value">{{ active() }}</span></div>
      ...
      <app-example-card title="Rejection-decoration via <code>*cngxTabRejectionIcon</code>"
        [subtitle]="_s1"
  ...
  <div class="event-grid" style="margin-top:var(--demo-grid-gap, 12px)">
    ...
    <div class="event-row"><span class="event-label">Commit attempts</span><span class="event-value">{{ busyAttempts() }}</span></div>
      ...
      <app-example-card title="Custom busy-spinner via <code>*cngxTabBusySpinner</code>"
        [subtitle]="_s2"
...
export class TabSlotOverridesDemoComponent {
  protected readonly _s0 = 'Replace the default <code>!</code> glyph with a counter pill driven by the tab\'s aggregator. Context is <code>{ tab }</code>; the badge only renders when <code>tab.errorAggregator()?.shouldShow()</code> is truthy, so the slot is purely additive — no visibility plumbing in consumer markup. Toggle the "profile invalid" checkbox to flip the badge.';
  protected readonly _s1 = 'Surface a commit rollback visually — when <code>commitAction</code> rejects, the presenter sets <code>lastFailedIndex</code> and the slot fires for the rejected tab. Context is <code>{ failedIndex, originLabel? }</code>. Click the "Security" tab; the first attempt rejects, the rejection-icon appears, and a retry succeeds.';
  protected readonly _s2 = 'Replace the default pulse-animation span with branded markup while a commit is in flight. Slot context is <code>{ tab, intendedIndex }</code>; the slot only fires when the tab matches <code>presenter.intendedIndex()</code> with <code>commitState.status() === \'pending\'</code>. Click "Review" to trigger an 800ms pessimistic commit — the spinner slot replaces the default chrome on the target tab.';
  protected readonly _srcHtml0 = `<cngx-tab-group [(activeIndex)]="active" aria-label="Slot-overrides — error badge">
    <ng-template cngxTabErrorBadge let-tab="tab">
      <span class="chip" style="background:#dc2626;color:#fff;font-size:0.7em;padding:0 6px">
        {{ tab.errorAggregator()?.errorCount() }}
      </span>
  ...
  protected readonly _srcTs0 = `import { CngxTab, CngxTabBusySpinner, CngxTabContent, CngxTabErrorBadge, CngxTabRejectionIcon, type CngxTabsCommitAction } from '@cngx/common/tabs';
import { CngxTabGroup } from '@cngx/ui/tabs';
import { CngxErrorAggregator, CngxErrorSource } from '@cngx/common/interactive';
  ...
  protected readonly _srcHtml1 = `<cngx-tab-group
    [(activeIndex)]="active"
    [commitAction]="commitAction"
    commitMode="optimistic"
    aria-label="Slot-overrides — rejection icon"
  ...
  protected readonly _srcTs1 = `import { CngxTab, CngxTabBusySpinner, CngxTabContent, CngxTabErrorBadge, CngxTabRejectionIcon, type CngxTabsCommitAction } from '@cngx/common/tabs';
import { CngxTabGroup } from '@cngx/ui/tabs';
import { CngxErrorAggregator, CngxErrorSource } from '@cngx/common/interactive';
  ...
  protected readonly _srcHtml2 = `<cngx-tab-group
    [(activeIndex)]="active"
    [commitAction]="slowCommit"
    commitMode="pessimistic"
    aria-label="Slot-overrides — busy spinner"
  ...
  protected readonly _srcTs2 = `import { CngxTab, CngxTabBusySpinner, CngxTabContent, CngxTabErrorBadge, CngxTabRejectionIcon, type CngxTabsCommitAction } from '@cngx/common/tabs';
import { CngxTabGroup } from '@cngx/ui/tabs';
import { CngxErrorAggregator, CngxErrorSource } from '@cngx/common/interactive';
