...
export const STORY: DemoSpec = {
  ...
  <div class="event-grid" style="margin-top:12px">
    ...
    },
    {
      ...
      subtitle:
        'Ten tabs inside a 600px-wide container — the strip cannot fit them all, so <code>[cngxMatTabs]</code> programmatically mounts a <code>&lt;cngx-tab-overflow&gt;</code> "More" button pinned to the trailing edge of <code>.mat-mdc-tab-header</code>. Click the More button to see the popover list of hidden tabs; clicking a hidden tab routes through <code>presenter.selectById(...)</code> so the cngx commit-action lifecycle, rejection decoration, and aggregator visuals stay coherent across the click. <strong>Auto-hide:</strong> when the cngx overflow molecule is mounted, Material\'s built-in pagination arrows (<code>&lt;</code> / <code>&gt;</code>) auto-hide via a library CSS rule — the More popover supersedes both, and the trailing pagination arrow would otherwise collide visually with the More button. Consumers who want both affordances visible can opt back in via theme-level CSS (see the README). Resize the browser to see the More button engage and disengage as the strip width crosses the overflow threshold.',
