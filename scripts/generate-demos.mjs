...
export function generateComponentFile(story, meta) {
  ...
  const setupStatements = story.sections
    .flatMap((s) => (s.setup ? [s.setup] : []))
    .map((stmt) => `  ${stmt}`);
...
async function main() {
  ...
  // Generate component files and index files for demos that have stories
  for (const demo of demos) {
    if (!demo.story) continue;
    const demoDir = join(DEMOS_ROOT, relative(DEMOS_ROOT, join(DEMOS_ROOT, demo.lib)));
    const actualDemoDir = demoDirs.find((d) => parseDemoPath(d, DEMOS_ROOT).name === demo.name);
    ...
    await writeFile(join(actualDemoDir, demo.componentFileName), componentContent);
    ...
    console.log(`Generated: ${demo.name}-demo`);
  }
}
