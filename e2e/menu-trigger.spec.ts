...
const ROUTE = '/#/common/interactive/menu/trigger/dropdown-menu';

test.describe('CngxMenuTrigger demo', () => {
  test('aria-haspopup=menu on trigger', async ({ page }) => {
    await page.goto(ROUTE);
    await expect(page.locator('.cngx-ex-artifact button')).toHaveAttribute('aria-haspopup', 'menu');
  });

  test('ArrowDown opens menu and highlights first', async ({ page }) => {
    await page.goto(ROUTE);
    const trigger = page.locator('.cngx-ex-artifact button');
    await trigger.focus();
