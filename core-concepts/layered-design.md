...
This insulation is what makes the dual-rendering pattern inside `@cngx/ui` possible at all: the brain lives in a pure-CNGX twin (`@cngx/ui/stepper`, `@cngx/ui/tabs`) at a level where Material cannot leak in, and the Material twin (`@cngx/ui/mat-stepper`, `@cngx/ui/mat-tabs`) is a thin bridge that re-uses the brain via host directives.
