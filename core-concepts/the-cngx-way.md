...
- CSS Anchor Positioning instead of `cdk/overlay` for the popover stack
- Hand-rolled listbox / menu / active-descendant on signals instead of `ListKeyManager`
- `createSelectionController<T>` instead of `SelectionModel`
...
This axiom dictates every line of code in the library.

Every directive and component is audited against these constraints:
...
CNGX uses two orthogonal axes:

- **Package levels** are the import budget enforced by Sheriff.
- **Atomic taxonomy** (atom / molecule / organism) describes how a unit is composed inside a lib.

See [Layered Design](layered-design.md) for the full table and the enforcement story.
...
**Only organisms are decompose-eligible.** The planned eject workflow splits an organism along two seams:

- **Brain** seam - host directive + DI tokens, stays in the lib and stays updateable.
- **Skin** seam - template + CSS, ejected into the consumer.

Atoms and molecules are terminal: they have no skin to eject.
